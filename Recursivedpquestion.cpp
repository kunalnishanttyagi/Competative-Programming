...
int send(int backward,int forward,int k,int final){
    // now i have to calculate answer for the shot on ith wall with power k

    if(arr[forward][k]!=-1) return arr[forward][k];
    if(forward==0 || k<=1) return arr[forward][k]= 1;
    // int ans=0;
    // ans=send(backward+1,forward-1,k,final)%mod;
    // ans=(ans+send(forward,backward,k-1,final)%mod)%mod;
    return arr[forward][k]=(send(backward+1,forward-1,k,final)+send(forward,backward,k-1,final))%mod;
}
...
memset(arr, -1, sizeof(arr));  // Fills with 0xFF bytes
    int ans=0;
    // if(k>1) ans++;
 cout<<ans+send(0,n,k,n)<<endl;
