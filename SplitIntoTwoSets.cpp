...
void dfs(int n,vi &value,vvi &v,map<int,int>&m,int root,vi &visited,int count) {
    // std::cout << root << " ";
    visited[root] = 1;
    int child=0,temp=0;
    ...
    for (int neighbor : v[root]) {
        ...
        if (!visited[neighbor]) {
            m[value[neighbor]]++;
            if(m[value[neighbor]]>=2){
              m[value[neighbor]]=1;
                // ans[value[neighbor]]=1;
                // visited[neighbor]=1;
                // cout<<value[neighbor]<<" ";
                return;
            }
            dfs(n,value,v,m,neighbor,visited,count+1);
        }
    }
    return;
}
...
bool send(double mid,int n,vector<double> &a,vector<double> &b){
    if(mid<0) return false;
    // now i will try to make ith position as mid
    // now i will make a range for first 
    // if(mid==1) cout<<"hii"<<endl;
    
    double low=(a[0]+b[0]-mid);
    double high=(a[0]-b[0]+mid);
    // if(b[0]>=mid){
        // if(n==1) return true;
        // return false;
        // low=b[0],high=
    // }
        ...
        double temp1=(a[i]+b[i]-mid);
        double temp2=(a[i]-b[i]+mid);
        if(temp1>low){
            low=temp1;
        }
        if(temp2<high) high=temp2;
    }
...
void solve(){
    ...
    set<int>pp;
                        ...
                        // if(count[x]>2) p=1;
                        if(count[i]>2) p=1;
        // now go to 1 and put the child in 
        // if i have reached and count is 0
        if(count[i]<2){
            ...
            while(!q.empty()){
                ...
                for(auto j:v[x]){
                    if(count[j]>2 || count[x]>2) p=1;
                    // cout<<i<<" "<<j<<endl;
                    if(count[j]>=2) continue;
                    // put the first one in the first set and other in second set
                    if(a.find(j)==a.end() && a.find(x)==a.end()){
                        a.insert(j);
                        a.insert(x);
                        count[x]++;
                        ...
                        if(count[x]>2) p=1;
                        if(count[j]>2) p=1;
                        if(count[j]<2 && pp.find(j)==pp.end())
                        q.push(j);
                        pp.insert(j);
                    }
                    else{
                        if(b.find(j)==b.end() && b.find(x)==b.end()){
                            b.insert(x);
                            b.insert(j);
                            count[x]++;
                            count[j]++;
